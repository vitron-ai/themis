...
describe('code scan generation', () => {
  ...
  function typecheckFixture(tempDir) {
    const packageDir = path.join(tempDir, 'node_modules', '@vitronai');
    const linkPath = path.join(packageDir, 'themis');
    fs.mkdirSync(packageDir, { recursive: true });
    if (!fs.existsSync(linkPath)) {
      fs.symlinkSync(REPO_ROOT, linkPath, 'dir');
    }

    const result = spawnSync(TSC_BIN, ['-p', path.join(tempDir, 'tsconfig.json'), '--pretty', 'false'], {
      cwd: REPO_ROOT,
      encoding: 'utf8'
    });
  ...
  test('generates deterministic tests, writes a source map artifact, and removes stale generated files', async () => {
    await withProjectFixture(
      ...
      async ({ tempDir, resolvePath }) => {
        ...
        expect(generatedSource.includes('// @ts-nocheck')).toBe(false);
        ...
        expect(widgetTestSource).toContain('/// <reference types="@vitronai/themis/globals" />');
        expect(widgetTestSource.includes('// @ts-nocheck')).toBe(false);
        expect(widgetTestSource.includes('toMatchSnapshot')).toBe(false);

        const mapPath = resolvePath('.themis', 'generate', 'generate-map.json');
        ...
        expect(rerun.removedFiles[0]).toContain('widget.generated.test.ts');
  ...
  test('supports planning mode, scenario steering, confidence steering, and persisted handoff artifacts', async () => {
    await withProjectFixture(
      ...
      async ({ tempDir, resolvePath }) => {
        ...
        const generateLastPath = resolvePath('.themis', 'generate', 'generate-last.json');
        const handoffPath = resolvePath('.themis', 'generate', 'generate-handoff.json');
  ...
  test('supports strict generation gates and writes a backlog artifact for unresolved work', async () => {
    await withProjectFixture(
      ...
      async ({ tempDir, resolvePath }) => {
        ...
        const backlogPath = resolvePath('.themis', 'generate', 'generate-backlog.json');
  ...
  test('supports one-command generation for a React-style TSX app', async () => {
    await withFixtureDirectory(
      ...
      async ({ tempDir, resolvePath }) => {
        ...
        expect(buttonTestSource).toContain('/// <reference types="@vitronai/themis/globals" />');
        expect(buttonTestSource.includes('// @ts-nocheck')).toBe(false);
  ...
  test('uses project-level fixture and mock providers from themis.generate.js', async () => {
    await withProjectFixture(
      ...
      async ({ tempDir, resolvePath }) => {
        ...
        expect(componentSource.includes("import path from 'path';")).toBe(false);
        expect(componentSource.includes('@vitronai/themis/contract-runtime')).toBe(true);
        expect(componentSource.includes('return import(SOURCE_IMPORT) as Promise<RuntimeModuleExports>;')).toBe(true);
        expect(componentSource.includes('const path = require(')).toBe(false);
        expect(componentSource).toContain('const SOURCE_FILE = require.resolve(SOURCE_IMPORT);');
  ...
  test('generated TypeScript tests typecheck in strict mode without a tsconfig types override', async () => {
    await withProjectFixture(
      {
        'package.json': `{\n  "name": "themis-ts-typecheck-fixture",\n  "private": true,\n  "version": "0.0.0"\n}\n`,
        'tsconfig.json': `{\n  "compilerOptions": {\n    "target": "ES2020",\n    "module": "CommonJS",\n    "strict": true,\n    "noEmit": true,\n    "skipLibCheck": true\n  }\n}\n`,
        'src/math.ts': `export function add(a: number, b: number) {\n  return a + b;\n}\n`
      },
      async ({ tempDir, resolvePath }) => {
        const summary = generateTestsFromSource(tempDir);
        const generatedPath = resolvePath('__themis__', 'tests', 'math.generated.test.ts');
        const generatedSource = fs.readFileSync(generatedPath, 'utf8');

        expect(summary.generatedFiles).toContain(generatedPath);
        expect(generatedSource).toContain('/// <reference types="@vitronai/themis/globals" />');
        expect(generatedSource.includes('// @ts-nocheck')).toBe(false);
        expect(generatedSource).toContain('const SOURCE_FILE = require.resolve(SOURCE_IMPORT);');

        const typecheck = typecheckFixture(tempDir);
  ...
  test('writes a structured fix handoff artifact for failed generated tests', async () => {
    await withProjectFixture(
      ...
      async ({ tempDir, resolvePath }) => {
        ...
        expect(agentPayload.artifacts.fixHandoff).toBe('.themis/runs/fix-handoff.json');
        expect(agentPayload.hints.repairGenerated).toBe('npx themis test --fix');

        const fixHandoff = JSON.parse(fs.readFileSync(resolvePath('.themis', 'runs', 'fix-handoff.json'), 'utf8'));
  ...
  test('repairs stale generated tests through npx themis test --fix', async () => {
    await withProjectFixture(
      {
        'src/math.js': `module.exports = {\n  add(a, b) {\n    return a + b;\n  }\n};\n`
      },
      async ({ tempDir, resolvePath }) => {
        ...
        let run = runCli(tempDir, ['test', '--json']);
        expect(run.status).toBe(1);

        run = runCli(tempDir, ['test', '--fix', '--json']);
        expect(run.status).toBe(0);
  ...
  test('emits rich machine-readable generation payload with entries, artifacts, prompt-ready output, and confidence tiers', async () => {
    await withFixtureDirectory(
      ...
      async ({ tempDir }) => {
        ...
        expect(payload.artifacts.generateMap).toBe('.themis/generate/generate-map.json');
        expect(payload.artifacts.generateResult).toBe('.themis/generate/generate-last.json');
        expect(payload.artifacts.generateHandoff).toBe('.themis/generate/generate-handoff.json');
